# 🟢 `parseArchitectMeta`

[← vscode-certify/src](../index.md)

---

## Identity

| Field | Value |
|-------|-------|
| **Unit ID** | `ts://vscode-certify/src/dataLoader.ts#parseArchitectMeta` |
| **Type** | function |
| **Path** | `vscode-certify/src/dataLoader.ts` |
| **Language** | ts |
| **Symbol** | `parseArchitectMeta` |

## Certification

| Field | Value |
|-------|-------|
| **Grade** | 🟢 **A** |
| **Score** | 93.1% |
| **Status** | certified |
| **Confidence** | 100% |
| **Certified** | 2026-09-07 |
| **Expires** | 2026-10-22 |
| **Source** | `deterministic` |

## Dimension Scores

| Dimension | Score | Bar |
|-----------|------:|-----|
| change_risk | 95.0% | ███████████████████░ |
| correctness | 95.0% | ███████████████████░ |
| maintainability | 95.0% | ███████████████████░ |
| operational_quality | 95.0% | ███████████████████░ |
| performance_appropriateness | 95.0% | ███████████████████░ |
| readability | 95.0% | ███████████████████░ |
| security | 85.0% | █████████████████░░░ |
| testability | 90.0% | ██████████████████░░ |

## Evidence

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 898/898 passed (67% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0.67 |
| `test_failed` | 0 |
| `test_passed` | 898 |
| `test_skipped` | 0 |
| `test_total` | 898 |

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 0/0 passed (0% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0 |
| `test_failed` | 0 |
| `test_passed` | 0 |
| `test_skipped` | 0 |
| `test_total` | 0 |

### ✅ git_history (`git`)

441 commits by 3 authors over 183 days

| Metric | Value |
|--------|------:|
| `age_days` | 183 |
| `author_count` | 3 |
| `commit_count` | 441 |

### ✅ metrics (`metrics`)

393 lines (329 code, 17 comment, 47 blank), 0 TODOs, complexity 0

| Metric | Value |
|--------|------:|
| `blank_lines` | 47 |
| `code_lines` | 329 |
| `comment_lines` | 17 |
| `complexity` | 0 |
| `todo_count` | 0 |
| `total_lines` | 393 |

### ✅ structural (`structural`)

structural: params=1 returns=1 nesting=1 doc=false exported=false cognitive=7

| Metric | Value |
|--------|------:|
| `cognitive_complexity` | 7 |
| `context_not_first` | 0 |
| `defer_in_loop` | 0 |
| `empty_catch_blocks` | 0 |
| `errors_ignored` | 0 |
| `errors_not_wrapped` | 0 |
| `exported_name` | 0 |
| `func_lines` | 27 |
| `global_mutable_count` | 0 |
| `hardcoded_secrets` | 0 |
| `has_doc_comment` | 0 |
| `has_init_func` | 0 |
| `is_constructor` | 0 |
| `loop_nesting_depth` | 0 |
| `max_nesting_depth` | 1 |
| `method_count` | 0 |
| `naked_returns` | 0 |
| `nested_loop_pairs` | 0 |
| `os_exit_calls` | 0 |
| `panic_calls` | 0 |
| `param_count` | 1 |
| `quadratic_patterns` | 0 |
| `recursive_calls` | 0 |
| `return_count` | 1 |
| `unsafe_import_count` | 0 |

---

*Repository: `iksnae/code-certification` · Commit: `623a65fe0` · Generated: 2026-09-07T13:51:15Z*
*Generated by [Certify](https://github.com/iksnae/code-certification)*
