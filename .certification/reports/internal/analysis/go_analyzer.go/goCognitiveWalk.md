# 🟢 `goCognitiveWalk`

[← internal/analysis](../index.md)

---

## Identity

| Field | Value |
|-------|-------|
| **Unit ID** | `go://internal/analysis/go_analyzer.go#goCognitiveWalk` |
| **Type** | function |
| **Path** | `internal/analysis/go_analyzer.go` |
| **Language** | go |
| **Symbol** | `goCognitiveWalk` |

## Certification

| Field | Value |
|-------|-------|
| **Grade** | 🟢 **B+** |
| **Score** | 89.4% |
| **Status** | certified |
| **Confidence** | 100% |
| **Certified** | 2026-09-07 |
| **Expires** | 2026-10-22 |
| **Source** | `deterministic` |

## Dimension Scores

| Dimension | Score | Bar |
|-----------|------:|-----|
| architectural_fitness | 95.0% | ███████████████████░ |
| change_risk | 95.0% | ███████████████████░ |
| correctness | 95.0% | ███████████████████░ |
| maintainability | 85.0% | █████████████████░░░ |
| operational_quality | 95.0% | ███████████████████░ |
| performance_appropriateness | 95.0% | ███████████████████░ |
| readability | 95.0% | ███████████████████░ |
| security | 85.0% | █████████████████░░░ |
| testability | 65.0% | █████████████░░░░░░░ |

## Evidence

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 898/898 passed (67% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0.67 |
| `test_failed` | 0 |
| `test_passed` | 898 |
| `test_skipped` | 0 |
| `test_total` | 898 |

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 0/0 passed (0% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0 |
| `test_failed` | 0 |
| `test_passed` | 0 |
| `test_skipped` | 0 |
| `test_total` | 0 |

### ✅ git_history (`git`)

441 commits by 3 authors over 183 days

| Metric | Value |
|--------|------:|
| `age_days` | 183 |
| `author_count` | 3 |
| `commit_count` | 441 |

### ✅ metrics (`metrics`)

5 lines (5 code, 0 comment, 0 blank), 0 TODOs, complexity 2

| Metric | Value |
|--------|------:|
| `blank_lines` | 0 |
| `code_lines` | 5 |
| `comment_lines` | 0 |
| `complexity` | 2 |
| `todo_count` | 0 |
| `total_lines` | 5 |

### ✅ test (`coverage:unit`)

per-unit coverage: 74%

| Metric | Value |
|--------|------:|
| `unit_test_coverage` | 0.74 |

### ✅ structural (`structural`)

structural: params=3 returns=0 nesting=1 doc=false exported=false cognitive=1

| Metric | Value |
|--------|------:|
| `cognitive_complexity` | 1 |
| `context_not_first` | 0 |
| `defer_in_loop` | 0 |
| `empty_catch_blocks` | 0 |
| `errors_ignored` | 0 |
| `errors_not_wrapped` | 0 |
| `exported_name` | 0 |
| `func_lines` | 3 |
| `global_mutable_count` | 0 |
| `hardcoded_secrets` | 0 |
| `has_doc_comment` | 0 |
| `has_init_func` | 1 |
| `is_constructor` | 0 |
| `loop_nesting_depth` | 1 |
| `max_nesting_depth` | 1 |
| `method_count` | 0 |
| `naked_returns` | 0 |
| `nested_loop_pairs` | 0 |
| `os_exit_calls` | 0 |
| `panic_calls` | 0 |
| `param_count` | 3 |
| `quadratic_patterns` | 0 |
| `recursive_calls` | 0 |
| `return_count` | 0 |
| `unsafe_import_count` | 0 |

### ✅ structural (`deep-analysis`)

deep: fan_in=12 fan_out=1 dead=false depth=1 instab=0.29

| Metric | Value |
|--------|------:|
| `concrete_deps` | 0 |
| `coupling_score` | 0.12 |
| `dep_depth` | 1 |
| `fan_in` | 12 |
| `fan_out` | 1 |
| `instability` | 0.29 |
| `interface_size` | 0 |
| `is_dead_code` | 0 |
| `type_aware_unwrapped` | 0 |
| `unused_params` | 0 |

## Observations

- has_init_func: 1 exceeds threshold 0

---

*Repository: `iksnae/code-certification` · Commit: `623a65fe0` · Generated: 2026-09-07T13:51:15Z*
*Generated by [Certify](https://github.com/iksnae/code-certification)*
