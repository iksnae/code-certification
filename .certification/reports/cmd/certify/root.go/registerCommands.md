# 🟢 `registerCommands`

[← cmd/certify](../index.md)

---

## Identity

| Field | Value |
|-------|-------|
| **Unit ID** | `go://cmd/certify/root.go#registerCommands` |
| **Type** | function |
| **Path** | `cmd/certify/root.go` |
| **Language** | go |
| **Symbol** | `registerCommands` |

## Certification

| Field | Value |
|-------|-------|
| **Grade** | 🟢 **B+** |
| **Score** | 90.0% |
| **Status** | certified |
| **Confidence** | 100% |
| **Certified** | 2026-09-06 |
| **Expires** | 2026-10-21 |
| **Source** | `deterministic` |

## Dimension Scores

| Dimension | Score | Bar |
|-----------|------:|-----|
| architectural_fitness | 65.0% | █████████████░░░░░░░ |
| change_risk | 95.0% | ███████████████████░ |
| correctness | 95.0% | ███████████████████░ |
| maintainability | 95.0% | ███████████████████░ |
| operational_quality | 95.0% | ███████████████████░ |
| performance_appropriateness | 95.0% | ███████████████████░ |
| readability | 95.0% | ███████████████████░ |
| security | 85.0% | █████████████████░░░ |
| testability | 90.0% | ██████████████████░░ |

## Evidence

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 898/898 passed (67% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0.67 |
| `test_failed` | 0 |
| `test_passed` | 898 |
| `test_skipped` | 0 |
| `test_total` | 898 |

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 0/0 passed (0% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0 |
| `test_failed` | 0 |
| `test_passed` | 0 |
| `test_skipped` | 0 |
| `test_total` | 0 |

### ✅ git_history (`git`)

439 commits by 3 authors over 181 days

| Metric | Value |
|--------|------:|
| `age_days` | 181 |
| `author_count` | 3 |
| `commit_count` | 439 |

### ✅ metrics (`metrics`)

31 lines (25 code, 4 comment, 2 blank), 0 TODOs, complexity 1

| Metric | Value |
|--------|------:|
| `blank_lines` | 2 |
| `code_lines` | 25 |
| `comment_lines` | 4 |
| `complexity` | 1 |
| `todo_count` | 0 |
| `total_lines` | 31 |

### ✅ test (`coverage:unit`)

per-unit coverage: 77%

| Metric | Value |
|--------|------:|
| `unit_test_coverage` | 0.77 |

### ✅ structural (`structural`)

structural: params=0 returns=0 nesting=0 doc=true exported=false cognitive=0

| Metric | Value |
|--------|------:|
| `cognitive_complexity` | 0 |
| `context_not_first` | 0 |
| `defer_in_loop` | 0 |
| `empty_catch_blocks` | 0 |
| `errors_ignored` | 0 |
| `errors_not_wrapped` | 0 |
| `exported_name` | 0 |
| `func_lines` | 26 |
| `global_mutable_count` | 0 |
| `hardcoded_secrets` | 0 |
| `has_doc_comment` | 1 |
| `has_init_func` | 0 |
| `is_constructor` | 0 |
| `loop_nesting_depth` | 0 |
| `max_nesting_depth` | 0 |
| `method_count` | 0 |
| `naked_returns` | 0 |
| `nested_loop_pairs` | 0 |
| `os_exit_calls` | 0 |
| `panic_calls` | 0 |
| `param_count` | 0 |
| `quadratic_patterns` | 0 |
| `recursive_calls` | 0 |
| `return_count` | 0 |
| `unsafe_import_count` | 0 |

### ✅ structural (`deep-analysis`)

deep: fan_in=1 fan_out=14 dead=false depth=3 instab=1.00

| Metric | Value |
|--------|------:|
| `concrete_deps` | 0 |
| `coupling_score` | 0.14 |
| `dep_depth` | 3 |
| `fan_in` | 1 |
| `fan_out` | 14 |
| `instability` | 1 |
| `interface_size` | 0 |
| `is_dead_code` | 0 |
| `type_aware_unwrapped` | 0 |
| `unused_params` | 0 |

---

*Repository: `iksnae/code-certification` · Commit: `d9dbb0026` · Generated: 2026-09-06T06:44:55Z*
*Generated by [Certify](https://github.com/iksnae/code-certification)*
