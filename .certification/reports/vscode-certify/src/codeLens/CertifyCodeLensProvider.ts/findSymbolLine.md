# 🟢 `findSymbolLine`

[← vscode-certify/src/codeLens](../index.md)

---

## Identity

| Field | Value |
|-------|-------|
| **Unit ID** | `ts://vscode-certify/src/codeLens/CertifyCodeLensProvider.ts#findSymbolLine` |
| **Type** | function |
| **Path** | `vscode-certify/src/codeLens/CertifyCodeLensProvider.ts` |
| **Language** | ts |
| **Symbol** | `findSymbolLine` |

## Certification

| Field | Value |
|-------|-------|
| **Grade** | 🟢 **B+** |
| **Score** | 88.8% |
| **Status** | certified |
| **Confidence** | 100% |
| **Certified** | 2026-07-23 |
| **Expires** | 2026-09-06 |
| **Source** | `deterministic` |

## Dimension Scores

| Dimension | Score | Bar |
|-----------|------:|-----|
| change_risk | 95.0% | ███████████████████░ |
| correctness | 95.0% | ███████████████████░ |
| maintainability | 95.0% | ███████████████████░ |
| operational_quality | 95.0% | ███████████████████░ |
| performance_appropriateness | 70.0% | ██████████████░░░░░░ |
| readability | 85.0% | █████████████████░░░ |
| security | 85.0% | █████████████████░░░ |
| testability | 90.0% | ██████████████████░░ |

## Evidence

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 748/748 passed (66% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0.66 |
| `test_failed` | 0 |
| `test_passed` | 748 |
| `test_skipped` | 0 |
| `test_total` | 748 |

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 0/0 passed (0% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0 |
| `test_failed` | 0 |
| `test_passed` | 0 |
| `test_skipped` | 0 |
| `test_total` | 0 |

### ✅ git_history (`git`)

382 commits by 3 authors over 136 days

| Metric | Value |
|--------|------:|
| `age_days` | 136 |
| `author_count` | 3 |
| `commit_count` | 382 |

### ✅ metrics (`metrics`)

154 lines (128 code, 4 comment, 22 blank), 0 TODOs, complexity 0

| Metric | Value |
|--------|------:|
| `blank_lines` | 22 |
| `code_lines` | 128 |
| `comment_lines` | 4 |
| `complexity` | 0 |
| `todo_count` | 0 |
| `total_lines` | 154 |

### ✅ structural (`structural`)

structural: params=3 returns=1 nesting=4 doc=false exported=false cognitive=22

| Metric | Value |
|--------|------:|
| `cognitive_complexity` | 22 |
| `context_not_first` | 0 |
| `defer_in_loop` | 0 |
| `empty_catch_blocks` | 0 |
| `errors_ignored` | 0 |
| `errors_not_wrapped` | 0 |
| `exported_name` | 0 |
| `func_lines` | 46 |
| `global_mutable_count` | 0 |
| `hardcoded_secrets` | 0 |
| `has_doc_comment` | 0 |
| `has_init_func` | 0 |
| `is_constructor` | 0 |
| `loop_nesting_depth` | 2 |
| `max_nesting_depth` | 4 |
| `method_count` | 0 |
| `naked_returns` | 0 |
| `nested_loop_pairs` | 0 |
| `os_exit_calls` | 0 |
| `panic_calls` | 0 |
| `param_count` | 3 |
| `quadratic_patterns` | 0 |
| `recursive_calls` | 0 |
| `return_count` | 1 |
| `unsafe_import_count` | 0 |

---

*Repository: `iksnae/code-certification` · Commit: `a410f4375` · Generated: 2026-07-23T05:20:01Z*
*Generated by [Certify](https://github.com/iksnae/code-certification)*
