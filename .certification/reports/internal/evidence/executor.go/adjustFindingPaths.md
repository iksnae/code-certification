# 🟢 `adjustFindingPaths`

[← internal/evidence](../index.md)

---

## Identity

| Field | Value |
|-------|-------|
| **Unit ID** | `go://internal/evidence/executor.go#adjustFindingPaths` |
| **Type** | method |
| **Path** | `internal/evidence/executor.go` |
| **Language** | go |
| **Symbol** | `adjustFindingPaths` |

## Certification

| Field | Value |
|-------|-------|
| **Grade** | 🟢 **B+** |
| **Score** | 89.4% |
| **Status** | certified |
| **Confidence** | 100% |
| **Certified** | 2026-09-10 |
| **Expires** | 2026-10-25 |
| **Source** | `deterministic` |

## Dimension Scores

| Dimension | Score | Bar |
|-----------|------:|-----|
| architectural_fitness | 95.0% | ███████████████████░ |
| change_risk | 95.0% | ███████████████████░ |
| correctness | 95.0% | ███████████████████░ |
| maintainability | 95.0% | ███████████████████░ |
| operational_quality | 95.0% | ███████████████████░ |
| performance_appropriateness | 95.0% | ███████████████████░ |
| readability | 95.0% | ███████████████████░ |
| security | 50.0% | ██████████░░░░░░░░░░ |
| testability | 90.0% | ██████████████████░░ |

## Evidence

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 898/898 passed (67% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0.67 |
| `test_failed` | 0 |
| `test_passed` | 898 |
| `test_skipped` | 0 |
| `test_total` | 898 |

### ✅ lint (`go vet`)

go vet: 0 errors, 0 warnings

| Metric | Value |
|--------|------:|
| `lint_errors` | 0 |
| `lint_warnings` | 0 |

### ✅ test (`go test`)

go test: 0/0 passed (0% coverage)

| Metric | Value |
|--------|------:|
| `test_coverage` | 0 |
| `test_failed` | 0 |
| `test_passed` | 0 |
| `test_skipped` | 0 |
| `test_total` | 0 |

### ✅ git_history (`git`)

444 commits by 3 authors over 185 days

| Metric | Value |
|--------|------:|
| `age_days` | 185 |
| `author_count` | 3 |
| `commit_count` | 444 |

### ✅ metrics (`metrics`)

18 lines (15 code, 3 comment, 0 blank), 0 TODOs, complexity 7

| Metric | Value |
|--------|------:|
| `blank_lines` | 0 |
| `code_lines` | 15 |
| `comment_lines` | 3 |
| `complexity` | 7 |
| `todo_count` | 0 |
| `total_lines` | 18 |

### ✅ test (`coverage:unit`)

per-unit coverage: 3%

| Metric | Value |
|--------|------:|
| `unit_test_coverage` | 0.03 |

### ✅ structural (`structural`)

structural: params=2 returns=0 nesting=2 doc=true exported=false cognitive=5

| Metric | Value |
|--------|------:|
| `cognitive_complexity` | 5 |
| `context_not_first` | 0 |
| `defer_in_loop` | 0 |
| `empty_catch_blocks` | 0 |
| `errors_ignored` | 0 |
| `errors_not_wrapped` | 0 |
| `exported_name` | 0 |
| `func_lines` | 13 |
| `global_mutable_count` | 0 |
| `hardcoded_secrets` | 0 |
| `has_doc_comment` | 1 |
| `has_init_func` | 0 |
| `is_constructor` | 0 |
| `loop_nesting_depth` | 1 |
| `max_nesting_depth` | 2 |
| `method_count` | 0 |
| `naked_returns` | 0 |
| `nested_loop_pairs` | 0 |
| `os_exit_calls` | 0 |
| `panic_calls` | 0 |
| `param_count` | 2 |
| `quadratic_patterns` | 0 |
| `recursive_calls` | 0 |
| `return_count` | 0 |
| `unsafe_import_count` | 1 |

### ✅ structural (`deep-analysis`)

deep: fan_in=5 fan_out=3 dead=false depth=1 instab=0.25

| Metric | Value |
|--------|------:|
| `concrete_deps` | 0 |
| `coupling_score` | 0.15 |
| `dep_depth` | 1 |
| `fan_in` | 5 |
| `fan_out` | 3 |
| `instability` | 0.25 |
| `interface_size` | 0 |
| `is_dead_code` | 0 |
| `type_aware_unwrapped` | 0 |
| `unused_params` | 0 |

## Observations

- unsafe_import_count: 1 exceeds threshold 0

---

*Repository: `iksnae/code-certification` · Commit: `bca621d2b` · Generated: 2026-09-10T06:54:42Z*
*Generated by [Certify](https://github.com/iksnae/code-certification)*
